  ...
  applyAuthoritativeEntities(entities: readonly PathwardenEntityState[]) {
    ...
    this.towers = entities.filter(entity => entity.type === 1).map(entity => {
      ...
      return {
        ...
        angle: 0,
        level: Number(components.level ?? 1),
        merges: 0,
        recoil: 0,
        targeting: (String(components.targeting ?? 'first') as PathwardenTargeting),
    ...
    this.enemies = entities.filter(entity => entity.type === 2).map(entity => {
      ...
      return {
        ...
        speed: Number(components.speed ?? 1),
        reward: Number(components.reward ?? 0),
        radius: enemyVisual[type].radius,
        slow: Number(components.slow ?? 0),
        slowTimer: Number(components.slowTimer ?? 0),
        healTimer: Number(components.healTimer ?? 0),
        color: enemyVisual[type].color,
        hitFlash: 0,
        attackTimer: 0,
        dotDamage: Number(components.dotDamage ?? 0),
        dotTimer: Number(components.dotTimer ?? 0),
        dotTick: 0
      }
    })
    this.projectiles = entities.filter(entity => entity.type === 3).map(entity => {
      ...
      return {
        ...
        relicFamily: components.relicFamily ? String(components.relicFamily) as PathwardenRelicFamily : undefined,
        relicPower: Number(components.relicPower ?? 0),
        echo: false,
        ...
        speed: Number(components.speed ?? 1),
        splash: Number(components.splash ?? 0),
        splashFactor: Number(components.splashFactor ?? 1),
        slow: Number(components.slow ?? 0),
