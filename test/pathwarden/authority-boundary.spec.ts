...
const pathwardenUtilsPath = resolve(root, 'server/utils/pathwarden.ts')
...
const pathwardenUtilsSource = readFileSync(pathwardenUtilsPath, 'utf8')
...
describe('Pathwarden authority boundaries', () => {
    it('keeps server-only modules out of the renderer and removes client-authored saves', () => {
        ...
        expect(realtimeSource).toContain('if (payload?.accepted) lastError.value = null')
        ...
        expect(engineSource).toContain('allowCommandsBeforeSnapshot')
        expect(componentSource).toContain('engine?.setAllowCommandsBeforeSnapshot(false)')
        ...
        expect(debugClearCacheSource).toContain('lastRunFinishedAt: null')
        expect(pathwardenUtilsSource).toContain('reconcileOrphanedPathwardenRun')
        expect(pathwardenUtilsSource).toContain('pathwardenRuns')
