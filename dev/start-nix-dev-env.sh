...
if [[ "${1:-}" != "--inside-nix-shell" ]]; then
    exec nix develop "path:$flake_dir" --command bash "$script_path" --inside-nix-shell "$@"
fi
...
if ! psql -h "$database_host" -p "$database_port" -U "$database_user" -d postgres \
    ...
    createuser -h "$database_host" -p "$database_port" -U "$database_user" --createdb --login "$database_user"
fi
...
if ! bun run db:migrate; then
    echo "db:migrate failed (likely schema drift). Recreating $database_name from scratch..."
    dropdb --if-exists --force -h "$database_host" -p "$database_port" -U "$database_user" "$database_name"
    createdb -h "$database_host" -p "$database_port" -U "$database_user" "$database_name"
    if ! bun run db:migrate; then
        echo "db:migrate failed again even after recreating the database." >&2
        exit 1
    fi
    echo "Database recreated from scratch and schema applied."
fi
